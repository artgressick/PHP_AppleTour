<?	
	...
	$title = 'Add Link';      // Title to display at the top of the browser window.
	...
	if(@$_POST['moveTo'] != "") { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		$q = "INSERT INTO Links SET 
			 chrName='".	encode($_POST['chrName']) ."',
			 idSeries='". 	$_POST['idSeries'] ."',
			 chrEvents='".	implode(",", $_POST['event']) ."',
			 chrSpecial='". $y[mt_rand(0,strlen($y))].$y[mt_rand(0,strlen($y))].$y[mt_rand(0,strlen($y))] ."',
...
<div class='instructions'>Fill out items and click submit.</div>
	...
	<div class='innerbody'>
		<table id='twoCol' class='twoCol' style='width: 100%;' cellpadding="0" cellspacing="0">
			<tr>			
				<td class="left" style="width:50%;">
					...
					<div class='FormField'><input type='text' name='chrName' id='chrName' size="50" maxlength="200" value="<?=$_POST['chrName']?>" /></div>
					<div class='FormName'>Event Series <span class='Required'>(Required)</span></div>
						...
						<? while ($row = mysqli_fetch_assoc($Series)) { ?>
							<option value='<?=$row['ID']?>'<?=($_POST['idSeries'] == $row['ID'] ? 'selected="selected"' : '')?>><?=$row['chrTitle']?></option>
						<?	} ?>
						</select>					
				</td>
				...
				while ($row = mysqli_fetch_assoc($Events)) { ?>
							<tr id='tr<?=$row['ID']?>' class='<?=($count++%2?'ListLineOdd':'ListLineEven')?>' 	onmouseover='RowHighlight("tr<?=$row['ID']?>");' onmouseout='UnRowHighlight("tr<?=$row['ID']?>");'>
								<td style="width:5px; vertical-align:middle;"><input type="checkbox" id="event<?=$row['ID']?>" name="event[]" value="<?=$row['ID']?>" /></td>
		...
		<input class='FormButtons' type='button' value='Add Link And Return' onclick="document.getElementById('moveTo').value='addlink.php'; error_check();" /> &nbsp;&nbsp; 
		<input class='FormButtons' type='button' value='Add And Move On' onclick="document.getElementById('moveTo').value='links.php'; error_check();" />
