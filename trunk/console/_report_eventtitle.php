...
// INSERT PROJECT NAME HERE
	...
	$workbook->send($filename.'_Event_Title_Report('. $time .').xls');	
...
function decode($val) {
	...
	// Create worksheet
	$worksheet =& $workbook->addWorksheet('Event_Title_Report('. $time .')');
	...
	$column_num++;
	$worksheet->setColumn($column_num, $column_num, 20);
	$worksheet->write($row_num, $column_num, 'Percent of Sign-ups', $format_column_header);
	$column_num++;
	$worksheet->setColumn($column_num, $column_num, 20);
	$worksheet->write($row_num, $column_num, 'Wait-listers to Date', $format_column_header);
	$column_num++;
	$worksheet->setColumn($column_num, $column_num, 20);
	$worksheet->write($row_num, $column_num, 'Cancellations to Date', $format_column_header);
	$column_num++;
	...
	while($row = mysqli_fetch_assoc($result)) {
		...
		$column_num++;	
		$worksheet->write($row_num, $column_num, $row['intPercent']."%", $format_data);
		$column_num++;
