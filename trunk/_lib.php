...
if(isset($_REQUEST['logout'])) {
	...
	session_unset();
	session_destroy();
	header("Location: ".$BF."console/index.php");
}
...
function error_report($message) {
	...
	mail(constant('BUG_REPORT_ADDRESS'), '[AppleAPPEvents] Error',
		"- ERROR\n----------------\n" . $message . "\n\n\n- STACK\n----------------\n" . $trace
		);
...
/* OLD ENCODE/DECODE
function encode_to_db($txt) {
   return htmlentities($txt,ENT_QUOTES,'UTF-8');
}

function decode_from_db($txt) {
   return html_entity_decode(html_entity_decode($txt,ENT_QUOTES,'UTF-8'),ENT_QUOTES,'UTF-8');
}
*/
...
function set_strs($str,$field_info,$info_old,$aud,$table,$id) { //This function does the additions to an update script
	$tmpStr = $tmpAud = "";
	if($info_old != encode($_POST[$field_info])) {
